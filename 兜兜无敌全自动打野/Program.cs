...
namespace JeonJunglePlay
{
    public class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            JeonAutoJungleMenu = new Menu("兜兜无敌全自动打野", "兜兜无敌全自动打野", true);
            JeonAutoJungleMenu.AddItem(new MenuItem("isActive", "开启")).SetValue(true);
            JeonAutoJungleMenu.AddItem(new MenuItem("maxstacks", "最大运行线程").SetValue(new Slider(30, 1, 150)));
            JeonAutoJungleMenu.AddItem(new MenuItem("autorecallheal", "自动补血")).SetValue(true);
            JeonAutoJungleMenu.AddItem(new MenuItem("autorecallitem", "自动加入团战")).SetValue(true);
            JeonAutoJungleMenu.AddItem(new MenuItem("evading", "防御塔攻击检测")).SetValue(true);
